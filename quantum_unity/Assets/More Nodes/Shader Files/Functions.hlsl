...
float rand3dTo1d(float3 value, float3 dotDir = float3(12.9898, 78.233, 37.719))
{
	...
	//get scalar value from 3d float3
	float random = dot(smallValue, dotDir);
...
float gnoise(float3 p)
{
	...
	float4 k1 = permin(b.xyxy);
	float4 k2 = permin(k1.xyxy + b.zzww);

	float4 c = k2 + a.zzzz;
	float4 k3 = permin(c);
	float4 k4 = permin(c + 1.0);
...
{
	float w = 0., d = .5;
	float4 y = float4(100, 100, 100, 100);
	for (int x = 0; x < NUM_OCTAVES; ++x)
		w += d * snoise(v), v = v * 2. + y, d *= .5;
	return w;
}
...
float2 swirl(float2 uv, float2 center, float angle)
{
	...
	float a = atan(offset) + angle * dist;
	return center + float2(cos(a), sin(a)) * dist;
}
...
float2 curlNoise(float2 uv)
{
	...
	float2 n = float2(noise(uv + dx), noise(uv + dy));
	return normalize(n) - 0.5;
}
float radialGradient(float2 uv, float2 center, float radius,float smooth)
{
	return 1.0 - smoothstep(smooth, radius, length(uv - center));
}
...
float3 BrickTexture(float2 uv, // Define parameters
float2 brickSize = float2(0.3, 0.15), // Width and height of each brick
float mortarThickness = 0.02, // Thickness of the mortar
float3 brickColor = float3(0.8, 0.3, 0.2), // Base color of the bricks
float3 mortarColor = float3(0.8, 0.8, 0.8), // Color of the mortar
float colorVariation = 0.1) // Amount of color variation in bricks)
{
    // Determine the position of the brick grid
	float2 position = uv / brickSize;

    // Offset every other row to create staggered bricks
	if (frac(position.y * 0.5) > 0.5)
		position.x += 0.5;

    // Calculate the brick ID
	float2 brickID = floor(position);
    
    // Determine the local position within the brick
	float2 brickUV = frac(position);

    // Add color variation based on the brick ID
	float3 brickColorWithVariation = brickColor * (1.0 + rand1(brickID) * colorVariation);

    // Calculate whether the current fragment is in the mortar or brick
	float isMortar = step(min(brickUV.x, brickUV.y), mortarThickness);

    // Mix the brick color and mortar color based on the calculated value
	float3 color = lerp(brickColorWithVariation, mortarColor, isMortar);

	return color;
}
float woodTexture(float2 uv)
{
	float radius = length(uv - 0.5);
	float angle = atan2(uv.y - 0.5, uv.x - 0.5);
	float woodPattern = sin(radius * 12.0 + angle * 5.0);
	return woodPattern;
}

  ...
  private void OnObservedGameUpdated(QuantumGame game) {
    ...
    if (game.Frames.Predicted != null) {
      ...
      foreach (var kvp in _activeViews) {
        ...
        {
          if (game.Frames.Predicted.Unsafe.TryGetPointer(instance.EntityRef, out CustomAnimator* animator))
          {
            instance.CustomQuantumAnimator.Animate(game.Frames.Predicted, animator);
          }
        }
      }
    }

...
public class CustomExportAnimations : MonoBehaviour
{
    ...
    public static void CreateAsset(CustomAnimatorGraphAsset dataAsset, AnimatorController controller)
    {
        ...
        for (int l = 0; l < layerCount; l++)
        {
            ...
            for (int s = 0; s < stateCount; s++)
            {
                ...
                {
                    if (state.behaviours[i] is AnimationEventContainer eventContainer)
                    {
                        eventContainer.Event.Settings.AnimID = newState.id;
    ...
    public static AnimatorData Extract(AnimationClip clip, bool isReversed)
    {
        ...
        {
            "LFoot" or "RFoot" or "Head" => true,
            _ => false
        }).ToArray();

        animationData.curves = new FPAnimationCurve[curves.Length];
        ...
        for (int i = 0; i < curves.Length; ++i)
        {
            ...
            {
                newKeys[j] = new(curve.keys[j].time * multiplier, curve.keys[j].value);
            }
            curve.keys = newKeys;
    ...
    private static void SaveQuantumCurve(AnimationCurve animationCurve, int resolution, ref FPAnimationCurve fpCurve, float startTime, float endTime)
    {
        if (resolution <= 0)
            return;

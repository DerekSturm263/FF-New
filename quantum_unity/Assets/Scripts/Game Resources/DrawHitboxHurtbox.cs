...
public class DrawHitboxHurtbox : Controller<DrawHitboxHurtbox>
{
    ...
    private unsafe void DrawAllHitboxes()
    {
        ...
        {
            transform = Matrix4x4.TRS((item.Transform->Position + item.HitboxInstance->Shape.Centroid).ToUnityVector3(), item.Transform->Rotation.ToUnityQuaternion(), 4 * item.HitboxInstance->Shape.BroadRadius.AsFloat * Vector3.one);

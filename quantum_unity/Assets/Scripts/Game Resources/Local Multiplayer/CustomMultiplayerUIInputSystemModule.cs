...
public class CustomMultiplayerUIInputSystemModule : MonoBehaviour
{
    ...
    private void Navigate(Vector2 dir)
    {
        ...
        if (QuantumRunner.Default.Game.Frames.Verified.TryGet(Quantum.FighterIndex.GetPlayerFromIndex(QuantumRunner.Default.Game.Frames.Verified, _playerInfo.Index), out Quantum.CharacterController characterController) && characterController.IsReady)
            return;
    ...
    public void BindControls(LocalPlayerInfo player)
    {
        ...
        _selector.Initialize(player);

        _playerInfo?.EnableMenus();
    }

    private void OnEnable() => _playerInfo?.EnableMenus();
    private void OnDisable() => _playerInfo?.DisableMenus();
}

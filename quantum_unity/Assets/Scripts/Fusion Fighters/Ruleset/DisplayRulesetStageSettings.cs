...
public class DisplayRulesetStageSettings : Display<Type, List<Selectable>>
{
    ...
    {
        (_component[0] as Toggle).isOn = item.AllowGizmos;
        (_component[1] as Toggle).isOn = item.AllowCustomStages;
        (_component[2] as Toggle).isOn = item.AllowDuplicateSelection;
    }

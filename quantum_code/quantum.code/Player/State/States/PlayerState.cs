namespace Quantum.Movement
{
    public unsafe abstract class PlayerState
    {
        ...
        protected abstract int StateTime(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats);
        protected virtual int DelayedEntranceTime(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats) => 0;
        ...
        public bool TryEnterAndResolveState(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
            ...
            if (filter.CharacterController->CanInput && GetInput(ref input) && DoesStateTypeMatch(ref filter) && !filter.CharacterController->IsHolding(GetState()) && CanEnter(f, ref filter, ref input, settings, stats))
            {
                ...
                for (int i = 0; i < killList.Length; ++i)
                {
                    if (filter.CharacterController->IsInState(killList[i]))
                        PlayerStateSystem.AllStates[killList[i]].Exit(f, ref filter, ref input, settings, stats);
                }
        ...
        public bool TryExitAndResolveState(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
        ...
        protected virtual bool CanEnter(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats) => true;

        protected virtual bool CanExit(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats) => false;

        protected virtual void Enter(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
        ...
        protected virtual void DelayedEnter(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
            Log.Debug($"Delayed entered state: {GetType()}");
        }

        public virtual void Update(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
        ...
        protected virtual void Exit(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats)
        {
            filter.CharacterController->StateTime = 0;
        ...
        public void ForceExit(Frame f, ref PlayerStateSystem.Filter filter, ref Input input, MovementSettings settings, ApparelStats stats) => Exit(f, ref filter, ref input, settings, stats);
    }
}
